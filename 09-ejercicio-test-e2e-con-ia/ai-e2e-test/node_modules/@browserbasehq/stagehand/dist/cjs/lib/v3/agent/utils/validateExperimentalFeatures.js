...
function validateExperimentalFeatures(options) {
    ...
    const isCuaMode = agentConfig?.mode !== undefined
    ...
    if (isCuaMode) {
        ...
        if (executeOptions?.excludeTools &&
        ...
        if (unsupportedFeatures.length > 0) {
            throw new sdkErrors_js_1.StagehandInvalidArgumentError(`${unsupportedFeatures.join(", ")} ${unsupportedFeatures.length === 1 ? "is" : "are"} not supported with CUA (Computer Use Agent) mode.`);
        }
    }
    ...
    const hasIntegrations = agentConfig?.integrations && agentConfig.integrations.length > 0;
    const hasTools = agentConfig?.tools && Object.keys(agentConfig.tools).length > 0;
...
//# sourceMappingURL=validateExperimentalFeatures.js.map