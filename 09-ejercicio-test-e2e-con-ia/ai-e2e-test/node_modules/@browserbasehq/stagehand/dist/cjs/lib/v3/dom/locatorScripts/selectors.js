...
exports.resolveCssSelector = resolveCssSelector;
exports.resolveCssSelectorPierce = resolveCssSelectorPierce;
exports.resolveTextSelector = resolveTextSelector;
exports.resolveXPathMainWorld = resolveXPathMainWorld;
const xpathResolver_js_1 = require("./xpathResolver.js");
const parseTargetIndex = (value) => {
    const num = Number(value ?? 0);
    if (!Number.isFinite(num) || num < 0)
        return 0;
    return Math.floor(num);
};
const collectCssMatches = (selector, limit) => {
    if (!selector)
        return [];
...
function resolveCssSelectorPierce(selectorRaw, targetIndexRaw) {
    const selector = String(selectorRaw ?? "").trim();
    if (!selector)
        return null;
    const targetIndex = parseTargetIndex(targetIndexRaw);
...
function resolveTextSelector(rawNeedle, targetIndexRaw) {
    ...
    const walkerFor = (root) => {
        try {
            const doc = root instanceof Document
                ? root
...
function resolveXPathMainWorld(rawXp, targetIndexRaw) {
    const targetIndex = parseTargetIndex(targetIndexRaw);
    return (0, xpathResolver_js_1.resolveXPathAtIndex)(rawXp, targetIndex, { pierceShadow: true });
}
