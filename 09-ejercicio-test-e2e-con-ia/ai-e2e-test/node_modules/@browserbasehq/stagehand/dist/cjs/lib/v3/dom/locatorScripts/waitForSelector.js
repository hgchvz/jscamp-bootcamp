 ...
 */
const deepQuerySelector = (root, selector, pierceShadow) => {
    ...
    while (queue.length > 0) {
        ...
        try {
            const ownerDoc = currentRoot instanceof Document
                ? currentRoot
 ...
 */
const deepXPathQuery = (xpath, pierceShadow) => {
    return (0, xpathResolver_js_1.resolveXPathFirst)(xpath, { pierceShadow });
};
/**
 * Find element by selector (CSS or XPath) with optional shadow DOM piercing.
 */
const findElement = (selector, pierceShadow) => {
    if (isXPath(selector)) {
 ...
 */
const setupShadowObservers = (callback, observers) => {
    const seenRoots = new WeakSet();
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!observeRoot) {
            ...
            timeoutId = setTimeout(() => {
                ...
                clearTimer();
                cleanup();
                reject(new Error(`waitForSelector: Timeout ${timeout}ms exceeded waiting for "${selector}" to be ${state}`));
            }, timeout);
            return;
        }
...
//# sourceMappingURL=waitForSelector.js.map