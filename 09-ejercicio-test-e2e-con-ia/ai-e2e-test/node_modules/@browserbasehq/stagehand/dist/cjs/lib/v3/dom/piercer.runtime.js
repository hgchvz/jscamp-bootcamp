...
exports.installV3ShadowPiercer = installV3ShadowPiercer;
function installV3ShadowPiercer(opts = {}) {
    // hardcoded debug (remove later if desired)
    const DEBUG = true;
    const bindBackdoor = (state) => {
        const { hostToRoot } = state;
        window.__stagehandV3__ = {
            getClosedRoot: (host) => hostToRoot.get(host),
            stats: () => ({
                installed: true,
                url: location.href,
    ...
    const currentFn = Element.prototype.attachShadow;
...
//# sourceMappingURL=piercer.runtime.js.map