...
exports.prettifySanitizeEvent = prettifySanitizeEvent;
exports.prettifyIsCdpEvent = prettifyIsCdpEvent;
exports.prettifyEvent = prettifyEvent;
exports.prettifyColorStderrLine = prettifyColorStderrLine;
...
function prettifySanitizeEvent(event) {
...
function prettifyIsCdpEvent(event) {
...
function prettifyCompactValue(value) {
    ...
    for (const [key, entry] of Object.entries(value)) {
        if (key === "page" ||
            ...
            key === "networkManager" ||
            key === "apiClient") {
            result[key] = `[${(0, utils_js_1.toTitleCase)(key)}]`;
            continue;
        }
...
function prettifyBuildContextTags(event, parentMap) {
    ...
    if (prettifyIsStagehandEvent(event)) {
        return [
            ...
            prettifyFormatTag(prettifyEventAction(stagehandEvent?.eventType ?? event.eventType).toUpperCase(), stagehandEvent?.eventId, "🆂"),
        ];
    }
...
async function prettifyEvent(store, event) {
...
function prettifyColorStderrLine(line) {
    ...
    return line
        ...
        .replace(/\[([🅰🆂🆄🅻🅲])([^\]]*)\]/gu, (_, icon, rest) => color(icon === "🅰"
        ? "36"
        : icon === "🆂"
            ? "33"
            : icon === "🆄"
