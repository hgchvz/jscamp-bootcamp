...
const index_js_2 = require("../types/private/index.js");
const actHandlerUtils_js_1 = require("./handlerUtils/actHandlerUtils.js");
...
class ActHandler {
    ...
    recordActMetrics(response) {
        this.onMetrics?.(methods_js_1.V3FunctionName.ACT, response.prompt_tokens ?? 0, response.completion_tokens ?? 0, response.reasoning_tokens ?? 0, response.cached_input_tokens ?? 0, response.inference_time_ms ?? 0);
    }
    async getActionFromLLM({ instruction, domElements, xpathMap, llmClient, requireMethodAndArguments = true, }) {
        const response = await (0, inference_js_1.act)({
            instruction,
            domElements,
            llmClient,
    ...
    async act(params) {
        ...
        ensureTimeRemaining();
        const { combinedTree, combinedXpathMap } = await (0, index_js_1.captureHybridSnapshot)(page, { experimental: true });
        const actInstruction = (0, prompt_js_1.buildActPrompt)(instruction, Object.values(index_js_2.SupportedUnderstudyAction), variables);
        ensureTimeRemaining();
        ...
        ensureTimeRemaining();
        const { combinedTree: combinedTree2, combinedXpathMap: combinedXpathMap2 } = await (0, index_js_1.captureHybridSnapshot)(page, {
            experimental: true,
        });
        let diffedTree = (0, index_js_1.diffCombinedTrees)(combinedTree, combinedTree2);
        if (!diffedTree.trim()) {
        ...
        const stepTwoInstructions = (0, prompt_js_1.buildStepTwoPrompt)(instruction, previousAction, Object.values(index_js_2.SupportedUnderstudyAction).filter((action) => action !== index_js_2.SupportedUnderstudyAction.SELECT_OPTION_FROM_DROPDOWN), variables);
        ensureTimeRemaining();
    ...
    async takeDeterministicAction(action, page, domSettleTimeoutMs, llmClientOverride, ensureTimeRemaining, variables) {
        ...
        try {
            ensureTimeRemaining?.();
            await (0, actHandlerUtils_js_1.performUnderstudyMethod)(page, page.mainFrame(), method, action.selector, resolvedArgs, settleTimeout);
            return {
                success: true,
            ...
            if (err instanceof sdkErrors_js_1.ActTimeoutError) {
                throw err;
            }
            ...
            if (this.selfHeal) {
                ...
                try {
                    ...
                    ensureTimeRemaining?.();
                    const { combinedTree, combinedXpathMap } = await (0, index_js_1.captureHybridSnapshot)(page, {
                        experimental: true,
                    });
                    const instruction = (0, prompt_js_1.buildActPrompt)(actCommand, Object.values(index_js_2.SupportedUnderstudyAction), {});
                    ensureTimeRemaining?.();
                    ...
                    ensureTimeRemaining?.();
                    await (0, actHandlerUtils_js_1.performUnderstudyMethod)(page, page.mainFrame(), method, newSelector, resolvedArgs, settleTimeout);
                    return {
                        success: true,
                    ...
                    if (retryErr instanceof sdkErrors_js_1.ActTimeoutError) {
                        throw retryErr;
                    }
...
function substituteVariablesInArguments(args, variables) {
    ...
    return args.map((arg) => {
        ...
        for (const [key, v] of Object.entries(variables)) {
            const token = `%${key}%`;
            out = out.split(token).join((0, variables_js_1.resolveVariableValue)(v));
        }
        return out;
    });
}
