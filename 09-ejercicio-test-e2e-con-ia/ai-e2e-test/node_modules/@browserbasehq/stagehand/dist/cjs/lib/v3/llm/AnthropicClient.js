...
class AnthropicClient extends LLMClient_js_1.LLMClient {
    type = "anthropic";
    client;
    ...
    async createChatCompletion({ options, retries, logger, }) {
        ...
        const formattedMessages = userMessages.map((msg) => {
            if (typeof msg.content === "string") {
                return {
                    role: msg.role, // ensure its not checking for system types
                    content: msg.content,
                };
            }
            else {
        ...
        let anthropicTools = options.tools?.map((tool) => {
            return {
                name: tool.name,
        ...
        if (options.response_model) {
            ...
            if (toolUse && "input" in toolUse) {
                ...
                throw new sdkErrors_js_1.CreateChatCompletionResponseError("No tool use with input in response");
            }
        }
...
const extractSchemaProperties = (jsonSchema) => {
