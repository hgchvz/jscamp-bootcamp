...
exports.shouldExpandNode = shouldExpandNode;
exports.mergeDomNodes = mergeDomNodes;
exports.collectDomTraversalTargets = collectDomTraversalTargets;
exports.hydrateDomTree = hydrateDomTree;
exports.getDomTreeWithFallback = getDomTreeWithFallback;
exports.domMapsForSession = domMapsForSession;
exports.buildSessionDomIndex = buildSessionDomIndex;
exports.relativizeXPath = relativizeXPath;
exports.findNodeByBackendId = findNodeByBackendId;
 ...
 */
async function hydrateDomTree(session, root, pierce) {
    const stack = [root];
        ...
        if (needsExpansion && (nodeId || backendId)) {
            ...
            for (const depth of DESCRIBE_DEPTH_ATTEMPTS) {
                try {
                    ...
                    throw new sdkErrors_js_1.StagehandDomProcessError(`Failed to expand DOM node ${identifier}: ${String(err)}`);
                }
            }
            if (!expanded) {
                const identifier = nodeId ?? backendId ?? "unknown";
                throw new sdkErrors_js_1.StagehandDomProcessError(`Unable to expand DOM node ${identifier} after describeNode depth retries`);
            }
        }
 ...
 */
async function getDomTreeWithFallback(session, pierce) {
    let lastCborMessage = "";
    ...
    throw new sdkErrors_js_1.StagehandDomProcessError(lastCborMessage
 ...
 */
async function domMapsForSession(session, frameId, pierce, encode, attemptOwnerLookup = true) {
    ...
    while (stack.length) {
        ...
        if (kids.length) {
            const segs = (0, xpathUtils_js_1.buildChildXPathSegments)(kids);
            for (let i = kids.length - 1; i >= 0; i--) {
                const child = kids[i];
                const step = segs[i];
                stack.push({
                    node: child,
                    xpath: (0, xpathUtils_js_1.joinXPath)(xpath, step),
                });
            }
        }
        for (const sr of node.shadowRoots ?? []) {
            stack.push({
                node: sr,
                xpath: (0, xpathUtils_js_1.joinXPath)(xpath, "//"),
            });
        }
    }
 ...
 */
async function buildSessionDomIndex(session, pierce) {
    await session.send("DOM.enable").catch(() => { });
    const root = await getDomTreeWithFallback(session, pierce);
    ...
    while (stack.length) {
        ...
        if (kids.length) {
            ...
            for (let i = kids.length - 1; i >= 0; i--) {
                const child = kids[i];
                const step = segs[i];
                stack.push({ node: child, xp: (0, xpathUtils_js_1.joinXPath)(xp, step), docRootBe });
            }
        }
        for (const sr of node.shadowRoots ?? []) {
            stack.push({ node: sr, xp: (0, xpathUtils_js_1.joinXPath)(xp, "//"), docRootBe });
        }
...
function relativizeXPath(baseAbs, nodeAbs) {
    const base = (0, xpathUtils_js_1.normalizeXPath)(baseAbs);
    const abs = (0, xpathUtils_js_1.normalizeXPath)(nodeAbs);
    if (abs === base)
        return "/";
