...
exports.collectFramesForScreenshot = collectFramesForScreenshot;
exports.normalizeScreenshotClip = normalizeScreenshotClip;
exports.computeScreenshotScale = computeScreenshotScale;
exports.setTransparentBackground = setTransparentBackground;
exports.applyStyleToFrames = applyStyleToFrames;
exports.disableAnimations = disableAnimations;
exports.hideCaret = hideCaret;
exports.applyMaskOverlays = applyMaskOverlays;
exports.runScreenshotCleanups = runScreenshotCleanups;
const sdkErrors_js_1 = require("../types/public/sdkErrors.js");
const screenshotScripts_generated_js_1 = require("../dom/build/screenshotScripts.generated.js");
...
function normalizeScreenshotClip(clip) {
    ...
    for (const [key, value] of Object.entries({ x, y, width, height })) {
        if (!Number.isFinite(value)) {
            throw new sdkErrors_js_1.StagehandInvalidArgumentError(`screenshot: clip.${key} must be a finite number`);
        }
    }
    if (width <= 0 || height <= 0) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError("screenshot: clip width/height must be positive");
    }
    return { x, y, width, height };
}
async function computeScreenshotScale(page, mode) {
    if (mode !== "css")
        return undefined;
    try {
...
async function setTransparentBackground(session) {
    await session
...
async function applyStyleToFrames(frames, css, label) {
    ...
    const token = `__v3_style_${label}_${Date.now()}_${Math.random()
        .toString(36)
        .slice(2)}`;
    await Promise.all(frames.map((frame) => frame
        .evaluate(({ css, token }) => {
        try {
            const doc = document;
            if (!doc)
...
async function disableAnimations(frames) {
    const css = `
*,
*::before,
*::after {
...
async function hideCaret(frames) {
    const css = `
input,
textarea,
[contenteditable],
...
async function applyMaskOverlays(locators, color) {
    ...
    await Promise.all(Array.from(rectsByFrame.entries()).map(([frame, { rects }]) => frame
        .evaluate(({ rects, color, token }) => {
        try {
            const doc = document;
            if (!doc)
                return;
            for (const rect of rects) {
                ...
                const root = rect.rootToken
                    ? doc.querySelector(`[data-stagehand-mask-root="${rect.rootToken}"]`) || defaultRoot
                    : defaultRoot;
                if (!root)
                    continue;
                if (rect.rootToken) {
                    try {
                        ...
                        if (style && style.position === "static") {
                            ...
                            if (!rootEl.hasAttribute("data-stagehand-mask-root-pos")) {
                                rootEl.setAttribute("data-stagehand-mask-root-pos", rootEl.style.position || "");
                            }
        ...
        .catch(() => { })));
    return async () => {
        await Promise.all(Array.from(rectsByFrame.entries()).map(([frame, { rootTokens }]) => frame
            .evaluate(({ token, rootTokens }) => {
            try {
                const doc = document;
                if (!doc)
                    return;
                ...
                for (const rootToken of rootTokens) {
                    const root = doc.querySelector(`[data-stagehand-mask-root="${rootToken}"]`);
                    if (!root)
                        continue;
...
async function resolveMaskRects(locator, maskToken) {
    const frame = locator.getFrame();
    const session = frame.session;
    try {
        const resolved = await locator.resolveNodesForMask();
        const rects = [];
...
async function resolveMaskRectForObject(session, objectId, maskToken) {
    const result = await session.send("Runtime.callFunctionOn", {
        objectId,
        functionDeclaration: screenshotScripts_generated_js_1.screenshotScriptSources.resolveMaskRect,
    ...
    return {
        ...
        width,
        height,
        rootToken: rect.rootToken && typeof rect.rootToken === "string"
            ? rect.rootToken
            : undefined,
    };
}
async function runScreenshotCleanups(cleanups) {
