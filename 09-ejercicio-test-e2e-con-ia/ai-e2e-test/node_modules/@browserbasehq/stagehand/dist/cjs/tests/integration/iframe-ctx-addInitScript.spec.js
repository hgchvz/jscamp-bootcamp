...
const parseBoundedTimeoutMs = (value, fallbackMs) => {
    const parsed = Number(value ?? fallbackMs);
    if (!Number.isFinite(parsed))
        return fallbackMs;
    return Math.max(MIN_TIMEOUT_MS, Math.min(MAX_TIMEOUT_MS, parsed));
};
const CHILD_FRAME_TIMEOUT_MS = parseBoundedTimeoutMs(process.env.IFRAME_CHILD_FRAME_TIMEOUT_MS, isBrowserbase ? 80_000 : 40_000);
const POPUP_TIMEOUT_MS = parseBoundedTimeoutMs(process.env.IFRAME_POPUP_TIMEOUT_MS, isBrowserbase ? 60_000 : 40_000);
const POPUP_URL_TIMEOUT_MS = parseBoundedTimeoutMs(process.env.IFRAME_POPUP_URL_TIMEOUT_MS, isBrowserbase ? 80_000 : 40_000);
...
const flattenFrameTree = (node, out = []) => {
    out.push({
        id: node.frame.id,
...
async function collectFrameSnapshot(page) {
    ...
    return Promise.all([...known.values()].map(async (frame) => {
        try {
...
async function logPageDiagnostics(page, reason, markerSelector) {
    if (!iframeDebugEnabled)
        return;
    const diagnostics = {
        reason,
    ...
    try {
        const domState = await page.mainFrame().evaluate((marker) => {
            ...
            const rect = el instanceof Element ? el.getBoundingClientRect().toJSON() : null;
            return {
                href: location.href,
                readyState: document.readyState,
                visibilityState: document.visibilityState,
    ...
    try {
        const frameTreeResponse = (await page.sendCDP("Page.getFrameTree"));
...
async function waitForChildFrame(page, expectedChildUrl, timeoutMs = CHILD_FRAME_TIMEOUT_MS) {
    ...
    const observedChildFrameIds = new Set();
    let lastUrl = "";
    ...
    while (Date.now() < deadline) {
        ...
        for (const childId of childIds)
            observedChildFrameIds.add(childId);
        ...
        if (childFrames.length) {
            const probes = await Promise.all(childFrames.map(async (child) => {
                try {
            ...
            const ready = probes.find((probe) => probe.readyState === "complete" && probe.href === expectedChildUrl);
            if (ready) {
    ...
    throw new Error(`Timed out waiting for child frame to load (timeout=${timeoutMs}ms, mainFrameId=${mainFrameId}, expectedChildUrl=${expectedChildUrl}, maxObservedFrames=${observedFrameCount}, observedChildFrameIds=[${[...observedChildFrameIds].join(",")}], url=${lastUrl || "<unknown>"})`);
}
async function waitForPageUrl(page, expectedUrlSubstring, timeoutMs = POPUP_URL_TIMEOUT_MS) {
    ...
    await logPageDiagnostics(page, `waitForPageUrl timeout for ${expectedUrlSubstring}`);
    throw new Error(`Timed out waiting for popup URL to include "${expectedUrlSubstring}" (timeout=${timeoutMs}ms, lastUrl=${lastUrl || "<unknown>"})`);
}
async function preparePopupForFrameAttach(page, markerSelector, timeoutMs = CHILD_FRAME_TIMEOUT_MS) {
    ...
    await logPageDiagnostics(page, "preparePopupForFrameAttach:ready", markerSelector);
}
async function ensurePopupViewport(page) {
    await page.setViewportSize(TEST_VIEWPORT.width, TEST_VIEWPORT.height);
    await logPageDiagnostics(page, "ensurePopupViewport");
}
async function waitForPopupPage(ctx, opener, timeoutMs = POPUP_TIMEOUT_MS) {
    ...
    const pageIds = ctx
        ...
        .join(", ");
    throw new Error(`Timed out waiting for popup page (timeout=${timeoutMs}ms, openerMainFrameId=${openerMainFrameId}, observedPages=[${pageIds}])`);
}
test_1.test.describe("context.addInitScript with iframes", () => {
    ...
    test_1.test.describe("direct navigation", () => {
        (0, test_1.test)("with OOPIF - sets background red in main page and iframe", async () => {
            const page = await ctx.newPage();
            await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/oopif-in-closed-shadow-dom/", { waitUntil: "networkidle" });
            ...
            (0, test_1.expect)(mainBgColor).toBe("rgb(255, 0, 0)");
            const iframeBgColor = await iframe.evaluate(() => {
                return getComputedStyle(document.documentElement).backgroundColor;
            });
            (0, test_1.expect)(iframeBgColor).toBe("rgb(255, 0, 0)");
        });
        (0, test_1.test)("with SPIF - sets background red in main page and iframe", async () => {
            const page = await ctx.newPage();
            await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/spif-in-closed-shadow-dom/", { waitUntil: "networkidle" });
    ...
    test_1.test.describe("via newPage", () => {
        (0, test_1.test)("with OOPIF - sets background red in main page and iframe", async () => {
            const page = await ctx.newPage();
            await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/oopif-in-closed-shadow-dom/", { waitUntil: "networkidle" });
            const iframe = await waitForChildFrame(page, OOPIF_CHILD_URL);
    ...
    test_1.test.describe("via popup", () => {
        (0, test_1.test)("with OOPIF - sets background red in main page and iframe", async () => {
            const page = await ctx.newPage();
            await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/ctx-add-init-script-oopif/", { waitUntil: "networkidle" });
            ...
            await waitForPageUrl(popup, "/stagehand-eval-sites/sites/oopif-in-closed-shadow-dom/");
            debugLog("popup-oopif:refresh-navigation", { url: popup.url() });
            await popup.goto("https://browserbase.github.io/stagehand-eval-sites/sites/oopif-in-closed-shadow-dom/", { waitUntil: "networkidle" });
            await logPageDiagnostics(popup, "popup-oopif:after-refresh", "shadow-host");
        ...
        (0, test_1.test)("with SPIF - sets background red in main page and iframe", async () => {
            const page = await ctx.newPage();
            await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/ctx-add-init-script-spif/", { waitUntil: "networkidle" });
            // Click link to open popup
            await page.locator("a").click();
            ...
            ctx.setActivePage(popup);
            await ensurePopupViewport(popup);
            await waitForPageUrl(popup, "/stagehand-eval-sites/sites/closed-shadow-dom-in-spif/");
...
//# sourceMappingURL=iframe-ctx-addInitScript.spec.js.map