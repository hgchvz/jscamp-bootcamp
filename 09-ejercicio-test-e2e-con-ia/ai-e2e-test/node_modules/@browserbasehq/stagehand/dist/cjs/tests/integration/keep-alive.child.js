...
async function main() {
    ...
    const { env, keepAlive, disableAPI, scenario, apiKey, projectId, debug = false, viewMs = 0, } = cfg;
    const log = (message) => {
        if (debug) {
            console.log(message);
        }
    ...
    log(`[keep-alive-child] env=${env} keepAlive=${keepAlive} disableAPI=${disableAPI} ` +
