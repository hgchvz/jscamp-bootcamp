...
test_1.test.describe("Page.dragAndDrop() - dragging elements", () => {
    let v3;
    ...
    (0, test_1.test)("drags and drops element to target zone", async () => {
        ...
        (0, test_1.expect)(resultText).toContain("DROP SUCCESSFUL");
    });
    (0, test_1.test)("drag and drop with steps parameter", async () => {
        const page = v3.context.pages()[0];
        await page.goto("data:text/html," +
            encodeURIComponent(`
          <!doctype html>
        ...
        (0, test_1.expect)(status).toContain("Dropped");
    });
    (0, test_1.test)("drag and drop with delay between steps", async () => {
        const page = v3.context.pages()[0];
        await page.goto("data:text/html," +
            encodeURIComponent(`
          <!doctype html>
        ...
        // Wait for events to be processed
        await page.evaluate(() => new Promise((r) => setTimeout(r, 100)));
        const isComplete = await page.evaluate(() => document.getElementById("complete").textContent === "true");
        (0, test_1.expect)(isComplete).toBe(true);
    });
    (0, test_1.test)("drag and drop returns xpath when requested", async () => {
        const page = v3.context.pages()[0];
        await page.goto("data:text/html," +
            encodeURIComponent(`
          <!doctype html>
        ...
        (0, test_1.expect)(typeof fromXpath).toBe("string");
        (0, test_1.expect)(typeof toXpath).toBe("string");
        (0, test_1.expect)(fromXpath.length).toBeGreaterThan(0);
        (0, test_1.expect)(toXpath.length).toBeGreaterThan(0);
    });
    (0, test_1.test)("drag and drop without returnXpath returns empty strings", async () => {
        const page = v3.context.pages()[0];
        await page.goto("data:text/html," +
            encodeURIComponent(`
          <!doctype html>
        ...
        (0, test_1.expect)(fromXpath).toBe("");
        (0, test_1.expect)(toXpath).toBe("");
    });
    (0, test_1.test)("drag and drop with different mouse buttons", async () => {
        const page = v3.context.pages()[0];
        await page.goto("data:text/html," +
            encodeURIComponent(`
          <!doctype html>
        ...
        // Wait for events to be processed
        await page.evaluate(() => new Promise((r) => setTimeout(r, 100)));
        const buttonUsed = await page.evaluate(() => document.getElementById("buttonUsed").textContent);
        (0, test_1.expect)(buttonUsed).toBe("left");
    });
    (0, test_1.test)("multiple sequential drag and drops", async () => {
        const page = v3.context.pages()[0];
        await page.goto("data:text/html," +
            encodeURIComponent(`
          <!doctype html>
        ...
        await page.evaluate(() => new Promise((r) => setTimeout(r, 100)));
        let dropCountText = await page.evaluate(() => document.getElementById("log").textContent);
        (0, test_1.expect)(dropCountText).toContain("Drops: 1");
        const item2Location = await page
            .frames()[0]
            .getLocationForSelector("#item2");
        ...
        // Wait for events to be processed
        await page.evaluate(() => new Promise((r) => setTimeout(r, 100)));
        dropCountText = await page.evaluate(() => document.getElementById("log").textContent);
        (0, test_1.expect)(dropCountText).toContain("Drops: 2");
    });
});
//# sourceMappingURL=page-drag-and-drop.spec.js.map