 ...
 */
const EXCLUDED_OUTPUT_KEYS = ["screenshotBase64"];
/**
...
function stripExcludedKeys(output) {
    ...
    for (const [key, value] of Object.entries(output)) {
        if (!EXCLUDED_OUTPUT_KEYS.includes(key)) {
            result[key] = value;
        }
    }
    return result;
}
export function mapToolResultToActions({ toolCallName, toolResult, args, reasoning, }) {
    switch (toolCallName) {
        case "act":
...
function createStandardAction(toolCallName, toolResult, args, reasoning) {
    const action = {
        type: toolCallName,
        reasoning,
        taskCompleted: toolCallName === "done" ? args?.taskComplete : false,
        ...args,
    };
    ...
    if (toolCallName !== "ariaTree" && toolResult) {
        ...
        if (output && typeof output === "object" && !Array.isArray(output)) {
            const cleanedOutput = stripExcludedKeys(output);
