...
export async function selectOption(ctx) {
...
async function scrollElementToPercentage(ctx) {
    const { locator, xpath, args } = ctx;
    v3Logger({
        category: "action",
...
async function scrollByPixelOffset(ctx) {
...
async function clickElement(ctx) {
    const { locator, xpath, args } = ctx;
    try {
        await locator.click({ button: args[0] || undefined });
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        v3Logger({
...
async function dragAndDrop(ctx) {
    ...
    if (!toXPath)
        throw new UnderstudyCommandException("dragAndDrop requires a target XPath arg");
...
async function scrollToPreviousChunk(ctx) {
    await scrollByElementHeight(ctx, /*dir=*/ -1);
}
async function scrollByElementHeight(ctx, direction) {
    const { locator, xpath } = ctx;
    v3Logger({
        category: "action",
    ...
    try {
        const ownerSession = locator.getFrame().session;
        await ownerSession.send("Runtime.callFunctionOn", {
            objectId,
            functionDeclaration: `
          function(dir) {
 ...
 */
export async function waitForDomNetworkQuiet(frame, timeoutMs) {
    const overallTimeout = typeof timeoutMs === "number" && Number.isFinite(timeoutMs)
    ...
    return new Promise((resolve) => {
        ...
        const onRequest = (p) => {
            ...
            if (p.type === "WebSocket" || p.type === "EventSource")
                return;
        ...
        const onDataUrl = (p) => {
            if (p.response.url?.startsWith("data:"))
                finishReq(p.requestId);
        };
        const onFrameStop = (f) => {
        ...
        const resolveDone = () => {
            ...
            if (quietTimer)
                clearTimeout(quietTimer);
            if (stalledRequestSweepTimer)
                clearInterval(stalledRequestSweepTimer);
            clearTimeout(guard);
            resolve();
        };
    });
}
//# sourceMappingURL=actHandlerUtils.js.map