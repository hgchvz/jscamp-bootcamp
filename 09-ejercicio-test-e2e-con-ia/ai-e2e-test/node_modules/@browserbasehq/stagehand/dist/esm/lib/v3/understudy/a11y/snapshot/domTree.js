 ...
 */
export function shouldExpandNode(node) {
...
export function mergeDomNodes(target, source) {
...
export function collectDomTraversalTargets(node) {
    const targets = [];
    if (node.children)
        ...
        if (needsExpansion && (nodeId || backendId)) {
            ...
            for (const depth of DESCRIBE_DEPTH_ATTEMPTS) {
                try {
                    ...
                    throw new StagehandDomProcessError(`Failed to expand DOM node ${identifier}: ${String(err)}`);
                }
            }
            if (!expanded) {
                const identifier = nodeId ?? backendId ?? "unknown";
                throw new StagehandDomProcessError(`Unable to expand DOM node ${identifier} after describeNode depth retries`);
            }
        }
 ...
 */
export async function getDomTreeWithFallback(session, pierce) {
    let lastCborMessage = "";
    ...
    throw new StagehandDomProcessError(lastCborMessage
 ...
 */
export async function domMapsForSession(session, frameId, pierce, encode, attemptOwnerLookup = true) {
 ...
 */
export async function buildSessionDomIndex(session, pierce) {
    await session.send("DOM.enable").catch(() => { });
    const root = await getDomTreeWithFallback(session, pierce);
 ...
 */
export function relativizeXPath(baseAbs, nodeAbs) {
