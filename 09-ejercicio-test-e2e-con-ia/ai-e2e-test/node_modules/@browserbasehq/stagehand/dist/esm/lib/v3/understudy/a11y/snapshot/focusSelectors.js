 ...
 */
export function parseXPathToSteps(path) {
    const s = path.trim();
    let i = 0;
    const steps = [];
    while (i < s.length) {
...
export function buildXPathFromSteps(steps) {
    let out = "";
 ...
 */
export async function resolveFocusFrameAndTail(page, absoluteXPath, parentByFrame, rootId) {
    ...
    const flushIntoChild = async () => {
        ...
        if (!objectId)
            throw new StagehandIframeError(selectorForIframe, "Failed to resolve iframe element by XPath");
        try {
...
export async function resolveCssFocusFrameAndTail(page, rawSelector, parentByFrame, rootId) {
...
export async function resolveObjectIdForXPath(session, xpath, frameId) {
    let contextId;
    try {
        if (frameId) {
...
export async function resolveObjectIdForCss(session, selector, frameId) {
    let contextId;
    try {
        if (frameId) {
            contextId = await executionContexts
...
export function listChildrenOf(parentByFrame, parentId) {
    const out = [];
