 ...
 */
export async function buildAbsoluteXPathFromChain(chain, leafSession, leafBackendNodeId) {
    let prefix = "";
 ...
 */
export async function absoluteXPathForBackendNode(session, backendNodeId) {
    try {
 ...
 */
export function prefixXPath(parentAbs, child) {
...
export function normalizeXPath(x) {
    if (!x)
        return "";
...
export function buildChildXPathSegments(kids) {
    const segs = [];
    const ctr = {};
...
export function joinXPath(base, step) {
    if (step === "//") {
        if (!base || base === "/")
            return "//";
