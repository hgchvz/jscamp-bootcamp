 ...
 */
export function filterCookies(cookies, urls) {
    if (!urls.length)
        return cookies;
 ...
 */
export function normalizeCookieParams(cookies) {
    return cookies.map((c) => {
        if (!c.url && !(c.domain && c.path)) {
            throw new CookieValidationError(`Cookie "${c.name}" must have a url or a domain/path pair`);
        }
        if (c.url && c.domain) {
            throw new CookieValidationError(`Cookie "${c.name}" should have either url or domain, not both`);
        }
        if (c.url && c.path) {
            throw new CookieValidationError(`Cookie "${c.name}" should have either url or path, not both`);
        }
        if (c.expires !== undefined && c.expires < 0 && c.expires !== -1) {
            throw new CookieValidationError(`Cookie "${c.name}" has an invalid expires value; use -1 for session cookies or a positive unix timestamp`);
        }
        const copy = { ...c };
        if (copy.url) {
        ...
        if (copy.sameSite === "None" && !copy.secure) {
            throw new CookieValidationError(`Cookie "${c.name}" has sameSite: "None" without secure: true. ` +
 ...
 * as session-scoped.
 */
export function toCdpCookieParam(c) {
    return {
        name: c.name,
        value: c.value,
        domain: c.domain,
 ...
 */
export function cookieMatchesFilter(cookie, options) {
