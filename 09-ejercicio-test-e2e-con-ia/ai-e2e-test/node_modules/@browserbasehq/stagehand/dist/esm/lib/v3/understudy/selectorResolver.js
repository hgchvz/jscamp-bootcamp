import { locatorScriptBootstrap, locatorScriptGlobalRefs, } from "../dom/build/locatorScripts.generated.js";
...
export class FrameSelectorResolver {
    ...
    static parseSelector(raw) {
        ...
        const looksLikeXPath = /^xpath=/i.test(trimmed) ||
            trimmed.startsWith("/") ||
            trimmed.startsWith("(");
    ...
    async resolveFirst(query) {
        return this.resolveAtIndex(query, 0);
    }
    async resolveAll(query, { limit = Infinity } = {}) {
        if (limit <= 0)
            return [];
        switch (query.kind) {
            case "css":
    ...
    async resolveAtIndex(query, index) {
    ...
    async resolveCss(selector, limit) {
        if (limit <= 0)
            return [];
        ...
        for (let index = 0; index < limit; index += 1) {
            ...
            const fallbackExpr = this.buildLocatorInvocation("resolveCssSelectorPierce", [JSON.stringify(selector), String(index)]);
    ...
    async resolveXPath(value, limit) {
        if (limit <= 0)
            return [];
        const session = this.frame.session;
        const ctxId = await executionContexts.waitForMainWorld(session, this.frame.frameId, 1000);
    ...
    async resolveFromObjectId(objectId) {
        const session = this.frame.session;
        let nodeId;
        try {
    ...
    async evaluateCount(expression, contextId) {
        const session = this.frame.session;
        try {
            const evalRes = await session.send("Runtime.evaluate", {
                expression,
    ...
    async evaluateElement(expression, contextId) {
        const session = this.frame.session;
        try {
            const evalRes = await session.send("Runtime.evaluate", {
                expression,
...
//# sourceMappingURL=selectorResolver.js.map