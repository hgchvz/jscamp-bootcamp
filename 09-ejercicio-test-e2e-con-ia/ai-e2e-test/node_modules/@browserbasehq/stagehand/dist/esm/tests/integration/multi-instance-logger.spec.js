...
test.describe("V3 Multi-Instance Logger Isolation", () => {
    ...
    test("V3 instances with external loggers don't leak logs to each other", async () => {
        const instance1Logs = [];
        const instance2Logs = [];
        const v3Instance1 = new V3(getV3DynamicTestConfig({
            verbose: 2,
            disablePino: true,
            logger: (line) => instance1Logs.push(line),
        }));
        const v3Instance2 = new V3(getV3DynamicTestConfig({
            verbose: 2,
            disablePino: true,
        ...
        try {
            ...
            expect(allMessages.size).toBeGreaterThanOrEqual(Math.max(instance1Messages.size, instance2Messages.size));
        }
        finally {
    ...
    test("rapidly creating and destroying instances doesn't cause logger issues", async () => {
        ...
        for (let i = 0; i < iterations; i++) {
            const logs = [];
            const v3 = new V3(getV3DynamicTestConfig({
