...
async function runCase(v3, c, framework) {
    ...
    try {
        ...
        for (const s of c.expectedSubstrings) {
            expect(text.includes(s), `expected pageText to include substring: ${s}`).toBeTruthy();
        }
    }
    finally {
        await cleanup?.();
