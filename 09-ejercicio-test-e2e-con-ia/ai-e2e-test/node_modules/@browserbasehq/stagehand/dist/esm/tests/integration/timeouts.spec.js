...
test.describe("V3 hard timeouts", () => {
    ...
    test("observe() enforces timeoutMs", async () => {
        ...
        await expect(v3.observe("find something", { timeout: 5 })).rejects.toThrow(/timed out/i);
    });
    test("extract() enforces timeoutMs", async () => {
        const schema = z.object({ title: z.string().optional() });
        await expect(v3.extract("Extract title", schema, { timeout: 5 })).rejects.toThrow(/timed out/i);
    });
