...
const makeContext = (sessions) => {
    const sessionsById = new Map(sessions.map((session) => [session.id, session]));
    return {
        _sessionInit: new Set(sessions.map((session) => session.id)),
        conn: {
            getSession: (id) => sessionsById.get(id),
        },
        extraHttpHeaders: null,
    };
};
describe("V3Context.setExtraHTTPHeaders", () => {
    const setExtraHTTPHeaders = V3Context.prototype.setExtraHTTPHeaders;
    ...
    it("throws a custom error with session failure details", async () => {
        ...
        await expect(promise).rejects.toBeInstanceOf(StagehandSetExtraHTTPHeadersError);
        try {
            await promise;
        }
        catch (error) {
...
//# sourceMappingURL=context-extra-http-headers.test.js.map