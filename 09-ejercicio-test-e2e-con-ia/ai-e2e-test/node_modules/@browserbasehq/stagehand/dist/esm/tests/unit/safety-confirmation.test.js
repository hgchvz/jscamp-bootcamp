...
const openAISafetyInvoker = OpenAICUAClient.prototype.handleSafetyConfirmation;
const googleSafetyInvoker = GoogleCUAClient.prototype.handleSafetyConfirmation;
function createOpenAIClient() {
    return new OpenAICUAClient("openai", "openai/computer-use-preview", "test instructions", { apiKey: "test" });
}
function createGoogleClient() {
    return new GoogleCUAClient("google", "google/gemini-2.5-computer-use-preview-10-2025", "test instructions", { apiKey: "test" });
}
describe("Safety Confirmation Handler", () => {
    ...
    describe("Google-style (safety_decision)", () => {
        ...
        it("returns 'true' when handler acknowledges", async () => {
            ...
            client.setSafetyConfirmationHandler(handler);
            const logger = vi.fn();
            const result = await googleSafetyInvoker.call(client, mockDecision, logger);
        ...
        it("handles string safety decisions", async () => {
            ...
            client.setSafetyConfirmationHandler(handler);
            const logger = vi.fn();
            const result = await googleSafetyInvoker.call(client, "Simple string decision", logger);
            expect(handler).toHaveBeenCalledWith([
                {
                    id: "google-safety-decision",
                    code: "safety_decision",
