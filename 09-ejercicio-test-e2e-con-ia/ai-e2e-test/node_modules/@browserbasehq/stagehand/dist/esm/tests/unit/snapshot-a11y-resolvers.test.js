...
describe("a11yForFrame", () => {
    ...
    it("scopes the tree to the resolved focus selector target", async () => {
        const nodes = baseAxNodes().map((n) => n.nodeId === "2"
            ? {
                ...n,
                childIds: ["3"],
            }
    ...
    it("falls back to full tree when resolveObjectId throws", async () => {
        ...
        vi.spyOn(focusSelectors, "resolveObjectIdForCss").mockRejectedValue(new Error("fail"));
        const opts = {
            focusSelector: ".btn",
            experimental: false,
            tagNameMap: {},
...
describe("resolveObjectIdForXPath", () => {
    ...
    it("evaluates in the target frame's main world when available", async () => {
        ...
        const objectId = await focusSelectors.resolveObjectIdForXPath(session, "//div", "frame-1");
        expect(objectId).toBe("node-obj");
    });
    it("returns null when evaluation throws or reports exception details", async () => {
        vi.spyOn(executionContexts, "waitForMainWorld").mockRejectedValue(new Error("missing"));
        vi.spyOn(executionContexts, "getMainWorld").mockReturnValue(undefined);
        const session = new MockCDPSession({
        ...
        const objectId = await focusSelectors.resolveObjectIdForXPath(session, "//div", "frame-2");
...
describe("resolveObjectIdForCss", () => {
    ...
    it("returns primary evaluation result when available", async () => {
        ...
        const objectId = await focusSelectors.resolveObjectIdForCss(session, ".btn", "frame-1");
    ...
    it("falls back to the pierce selector when the primary lookup fails", async () => {
        ...
        const session = new MockCDPSession({
            "Runtime.evaluate": async (params) => {
                ...
                expect(String(params?.expression)).toContain("resolveCssSelectorPierce");
                return { result: { objectId: "css-obj" } };
            },
        });
        const objectId = await focusSelectors.resolveObjectIdForCss(session, ".btn", undefined);
    ...
    it("returns null when both primary and fallback evaluations throw", async () => {
        ...
        const objectId = await focusSelectors.resolveObjectIdForCss(session, ".missing", "frame-1");
        expect(objectId).toBeNull();
    });
});
describe("tryScopedSnapshot", () => {
    ...
    it("returns scoped snapshot when focus selector resolves via CSS hops", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".btn" }, context, true);
        expect(result).not.toBeNull();
        expect(result?.combinedXpathMap["1-10"]).toBe("/html/body/iframe[1]/div[1]");
    ...
    it("returns null and logs fallback when scope is not applied", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".btn" }, context, false);
        expect(result).toBeNull();
        expect(loggerSpy).toHaveBeenCalled();
    });
    it("returns null immediately when no focus selector is provided", async () => {
        const result = await tryScopedSnapshot(makePage(new MockCDPSession({})), {}, context, true);
        expect(result).toBeNull();
    });
    it("supports XPath focus resolution branch", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: "xpath=//div" }, context, true);
        expect(result).not.toBeNull();
        expect(result?.combinedXpathMap["1-10"]).toBe("/div[1]");
    });
    it("logs and returns null when resolver throws", async () => {
        const session = new MockCDPSession({});
        vi.spyOn(focusSelectors, "resolveCssFocusFrameAndTail").mockRejectedValue(new Error("bad selector"));
        const loggerSpy = vi.spyOn(logger, "v3Logger").mockImplementation(() => { });
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".bad" }, context, true);
        expect(result).toBeNull();
        expect(loggerSpy).toHaveBeenCalled();
    });
});
