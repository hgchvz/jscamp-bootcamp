...
import { buildSessionDomIndex, domMapsForSession, getDomTreeWithFallback, hydrateDomTree, } from "../../lib/v3/understudy/a11y/snapshot/domTree.js";
...
describe("hydrateDomTree", () => {
    ...
    it("throws StagehandDomProcessError after exhausting describeNode retries", async () => {
        ...
        await expect(hydrateDomTree(session, root, true)).rejects.toBeInstanceOf(StagehandDomProcessError);
    });
});
describe("getDomTreeWithFallback", () => {
    ...
    it("throws StagehandDomProcessError when all depth attempts hit CBOR limits", async () => {
        ...
        await expect(getDomTreeWithFallback(session, false)).rejects.toBeInstanceOf(StagehandDomProcessError);
    });
});
describe("buildSessionDomIndex", () => {
    it("collects absolute paths, scrollability, and content-document metadata", async () => {
        ...
        expect(index.absByBe.get(tree.scrollDiv.backendNodeId)).toBe("/html[1]/body[1]/div[1]");
        expect(index.scrollByBe.get(tree.scrollDiv.backendNodeId)).toBe(true);
        expect(index.docRootOf.get(tree.iframeHtml.backendNodeId)).toBe(tree.iframeDoc.backendNodeId);
        expect(index.contentDocRootByIframe.get(tree.iframeElement.backendNodeId)).toBe(tree.iframeDoc.backendNodeId);
    });
});
describe("domMapsForSession", () => {
    it("derives frame-relative xpath/tag/scrollable maps for a frame's document root", async () => {
        ...
        const maps = await domMapsForSession(session, "frame-A", true, encode, true);
        ...
        expect(Object.keys(maps.tagNameMap)).not.toContain(`frame-A-${tree.html.backendNodeId}`);
    });
    it("falls back to the root document when frame owner lookup fails", async () => {
        ...
        const encode = (frameId, backendNodeId) => `${frameId}-${backendNodeId}`;
        const maps = await domMapsForSession(session, "frame-B", false, encode, true);
        expect(maps.xpathMap[`frame-B-${tree.html.backendNodeId}`]).toBe("/html[1]");
        expect(maps.xpathMap[`frame-B-${tree.scrollDiv.backendNodeId}`]).toBe("/html[1]/body[1]/div[1]");
    });
});
