...
import { computeFramePrefixes, mergeFramesIntoSnapshot, } from "../../lib/v3/understudy/a11y/snapshot/capture.js";
const makePage = (sessions) => ({
    getSessionForFrame: (frameId) => sessions[frameId] ?? sessions.root,
    getOrdinal: (frameId) => frameId === "frame-1" ? 0 : frameId === "frame-2" ? 1 : 2,
});
describe("computeFramePrefixes", () => {
    it("derives prefixes from parent iframe xpaths within the same session", async () => {
        ...
        const { absPrefix, iframeHostEncByChild } = await computeFramePrefixes(page, context, perFrameMaps, context.frames);
    ...
    it("inherits the parent prefix when frame owner lookups fail (OOPIF)", async () => {
        ...
        const maps = await computeFramePrefixes(page, context, perFrameMaps, context.frames);
    ...
    it("inherits parent prefix when iframe xpath mapping is missing", async () => {
        ...
        const result = await computeFramePrefixes(page, context, perFrameMaps, context.frames);
    ...
    it("does not compute prefixes for frames excluded from the scope", async () => {
        ...
        const { absPrefix, iframeHostEncByChild } = await computeFramePrefixes(page, context, perFrameMaps, ["frame-1"]);
...
describe("mergeFramesIntoSnapshot", () => {
    it("merges root and child maps, prefixing child xpaths and injecting subtrees", () => {
        ...
        const snapshot = mergeFramesIntoSnapshot(context, perFrameMaps, perFrameOutlines, absPrefix, iframeHostEncByChild, context.frames);
        expect(snapshot.combinedXpathMap["0-10"]).toBe("/html[1]/body[1]");
        expect(snapshot.combinedXpathMap["1-20"]).toBe("/html[1]/body[1]/iframe[1]/div[1]/span[1]");
    ...
    it("skips frames without maps and handles missing iframe mappings", () => {
        ...
        const snapshot = mergeFramesIntoSnapshot(context, perFrameMaps, perFrameOutlines, absPrefix, new Map(), context.frames);
    ...
    it("falls back to first outline when root frame outline is missing", () => {
        ...
        const snapshot = mergeFramesIntoSnapshot(context, perFrameMaps, perFrameOutlines, new Map([["frame-2", "/iframe[1]"]]), new Map(), context.frames);
    ...
    it("overwrites duplicate iframe host entries when multiple children map to the same parent", () => {
        ...
        const snapshot = mergeFramesIntoSnapshot(context, perFrameMaps, perFrameOutlines, new Map([
            ["frame-1", ""],
            ["frame-2", ""],
            ["frame-3", ""],
        ]), new Map([
    ...
    it("only merges xpath and url maps for frames included in frameIds", () => {
        ...
        const snapshot = mergeFramesIntoSnapshot(context, perFrameMaps, perFrameOutlines, new Map([["frame-1", ""]]), new Map(), ["frame-1"]);
        expect(snapshot.combinedXpathMap["0-10"]).toBe("/html[1]");
        expect(snapshot.combinedXpathMap["1-20"]).toBeUndefined();
        expect(snapshot.perFrame?.map((pf) => pf.frameId)).toEqual(["frame-1"]);
    });
});
//# sourceMappingURL=snapshot-frame-merge.test.js.map