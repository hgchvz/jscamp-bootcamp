...
import { cleanText, diffCombinedTrees, formatTreeLine, indentBlock, injectSubtrees, normaliseSpaces, } from "../../lib/v3/understudy/a11y/snapshot/treeFormatUtils.js";
describe("formatTreeLine", () => {
    it("includes encoded ids and indents children", () => {
        ...
        expect(outline).toBe("[frame-1] section: Container\n  [ax-2] button: Submit");
    });
});
describe("injectSubtrees", () => {
    it("nests child outlines under iframe encoded ids", () => {
        ...
        const merged = injectSubtrees(rootOutline, new Map([
            ["iframe-1", iframeOutline],
            ["nested-frame", nestedOutline],
        ]));
        expect(merged).toBe(`[root] document
  [iframe-1] iframe
    [child-root] child
      [nested-frame] iframe
        [nested-leaf] nested
  ...
  [iframe-1] iframe`;
        const iframeOutline = `[child-root] child`;
        const merged = injectSubtrees(rootOutline, new Map([["iframe-1", iframeOutline]]));
        expect(merged).toBe(`[root] document
  [iframe-1] iframe
    [child-root] child
  [iframe-1] iframe`);
    ...
    it("returns the original outline when no encoded ids are matched", () => {
        ...
        expect(injectSubtrees(outline, new Map([["other", "[x] child"]]))).toBe(outline);
    });
});
