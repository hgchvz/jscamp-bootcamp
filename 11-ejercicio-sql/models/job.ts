...
export class JobModel {
  ...
  static async create(input: CreateJobDTO): Promise<Job> {
    ...
    if (input.content) {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(id, id, input.content.description, input.content.responsibilities, input.content.requirements, input.content.about)
    }
  ...
  static async update(id: string, input: UpdateJobDTO): Promise<Job | null> {
    ...
    if (input.content && updated.content) {
      db.prepare(`
        INSERT INTO job_content (id, job_id, description, responsibilities, requirements, about)
        VALUES (?, ?, ?, ?, ?, ?)
      ...
      `).run(id, id, updated.content.description, updated.content.responsibilities, updated.content.requirements, updated.content.about)
    }
